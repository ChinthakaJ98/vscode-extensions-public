...
export async function generateCodeCore(params: GenerateCodeRequest, eventHandler: CopilotEventHandler): Promise<void> {
    ...
    const relevantTrimmedFuncs: Library[] = (await getRelevantLibrariesAndFunctions({query:prompt}, GenerationType.CODE_GENERATION)).libraries;
...
export async function repairCode(params: RepairParams): Promise<RepairResponse> {
    ...
    const allMessages: CoreMessage[] = [
        ...
        {
            role: "user",
            content: "Generated code returns following errors. Double-check all functions, types, record field access against the API documentation again. Fix the compiler errors and return the new response. \n Errors: \n " + params.diagnostics.map(d => d.message).join("\n")
        }
    ];
